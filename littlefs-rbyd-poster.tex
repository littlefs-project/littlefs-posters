...
increasing storage density---potentially GiBs of storage with
proportionally large block sizes.
...
Previous versions of \lfs+ avoided B-trees for this reason~\cite{littlefs2},
...
only when the node needs compaction.
...
Rbyd B-trees are currently implemented as primary file data-structure in
\lfsiii+. They have survived significant testing---including assertion
...
performance actually improves due to the increased branching-factor.

However, the more complex file data-structure is not without downsides.
Static analysis shows \lfsiii+'s code increasing from
16.9KiB {$\rightarrow$} 35.8KiB, and RAM increasing from
2.0KiB {$\rightarrow$} 2.9KiB.
